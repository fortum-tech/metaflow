...
class Batch(object):
    ...
    def create_job(
        ...
        job.job_name(job_name).job_queue(queue).command(
            self._command(code_package_url, self.environment, step_name, [step_cli])
        ).image(image).iam_role(iam_role).job_def(image, iam_role).cpu(cpu).gpu(
            gpu
        ).memory(
            memory
        ).timeout_in_secs(
        ...
        # when METAFLOW_DATASTORE_SYSROOT_LOCAL is NOT set (see
